...
def get_markov_matrix(videos='12345', include_same=False, frac=None):
    ...
    for n in videos:
        time, timeline = create_timeline(n)
        if frac is not None:
            tc = np.load('data_video_'+n+'.npz', allow_pickle=True)['threshold_change'].item()['changetime']
            if frac == 'before':
...
def markov_2(videos='12345'):
    ...
    for k, frac in enumerate(['before', 'after']):
        ...
        mc = MarkovChain(collapsed_mm,
                         ['social\nforaging', 'travel\ntowards', 'travel\naway', 'non social\nor out of patch'])
        mc.draw(img_path='markov_v2'+frac)
        ...
        for i, t in enumerate(time):
            ...
            if b is not None:
                ...
                if b in ['direct_competition', 'close_by', 'travel_towards', 'travel_away']:
                    bhv = 'other_patch_related'
                elif b in ['attack', 'separate_exploration', 'investigation']:
                    bhv = 'other_non_patch_related'
                else:
                    bhv = b
                same_patch = 0
